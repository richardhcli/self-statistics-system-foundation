...
const TopologyManager: React.FC = () => {
  ...
  const { deleteNode, upsertNode } = useCdagTopologyActions();
  const [newLabel, setNewLabel] = useState("");
  const [isAdding, setIsAdding] = useState(false);
  const handleAdd = () => { if (newLabel.trim()) { upsertNode(newLabel.trim(), { parents: {}, type: 'none' }); setNewLabel(""); setIsAdding(false); } };
  return (
    <div className="bg-white border-2 border-slate-900 p-6 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] rounded-2xl">
      ...
      <div className="space-y-3">
        ...
        {Object.entries(topology).map(([label, nodeData]) => (
          <div key={label} className="group border-2 border-slate-100 rounded-xl p-4 flex items-center justify-between">
            ...
            <button onClick={() => deleteNode(label)} className="p-2 text-slate-400 hover:text-red-600"><Trash2 className="w-4 h-4"/></button>
          </div>
        ))}
      </div>
    </div>
