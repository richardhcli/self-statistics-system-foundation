...
const TopologyManager: React.FC<TopologyManagerProps> = ({ data, deleteTopologyNode, addTopologyNode }) => {
  const [newLabel, setNewLabel] = useState("");
  const [isAdding, setIsAdding] = useState(false);
  const handleAdd = () => { if (newLabel.trim()) { addTopologyNode(newLabel.trim()); setNewLabel(""); setIsAdding(false); } };
  return (
    <div className="bg-white border-2 border-slate-900 p-6 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] rounded-2xl">
      ...
      <div className="space-y-3">
        {/* Use explicit casting for nodeData to resolve unknown property access */}
        {Object.entries(data.cdagTopology).map(([label, nodeData]) => (
          <div key={label} className="group border-2 border-slate-100 rounded-xl p-4 flex items-center justify-between">
            <div className="space-y-1"><h4 className="font-black text-slate-800 uppercase flex items-center gap-2">{label}<span className="text-[8px] px-1.5 py-0.5 bg-slate-100 text-slate-400 rounded uppercase">{Object.keys((nodeData as any).parents).length} PARENTS</span></h4></div>
            <button onClick={() => deleteTopologyNode(label)} className="p-2 text-slate-400 hover:text-red-600"><Trash2 className="w-4 h-4"/></button>
          </div>
        ))}
      </div>
    </div>
